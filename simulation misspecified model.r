...
for (i in 1:s.true){
u.sim1 <- rtweedie(length(u), mu = c(u), phi =  start.upper(claim, 2)$phi.u, power = 2) 
v.sim1 <- log(u.sim1)
...
beta10 <- hglm(fixed=hy_ij~as.factor(triangle_upper$dev),random=~1|as.factor(triangle_upper$origin), 
